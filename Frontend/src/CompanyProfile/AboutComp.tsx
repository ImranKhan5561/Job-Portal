    ...
    <div className="flex flex-col gap-5">
        {
          ...
          { key!="Website" && <div className="text-sm text-mine-shaft-300 text-justify">
              {key!="Specialities"?company[key]:company[key].map((item:string,index:number)=><span key={index}> &bull; {item}</span>)}
            </div>}
          { key=="Website"&& <a href={company[key] } target="_blank" className="text-sm text-bright-sun-400 text-justify">
              {company[key]}
            </a>}
